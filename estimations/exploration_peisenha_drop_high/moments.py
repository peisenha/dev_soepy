...
def get_moments(df):
    ...
    df_probs = df.groupby(conditioning[:2]).Choice.value_counts(normalize=True).rename("Value")
    ...
    df_wage = df_sim_working.groupby(conditioning)["Wage_Observed"].mean().rename("Value")
